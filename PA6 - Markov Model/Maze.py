...
class Maze:
    ...
    def test_valid(self, x, y):
        if x >= 0 and y >= 0 and x < self.width and y < self.height and self.lines[y][x] in {'r','g','b','y'}:
            return True
        return False
    
    def move(self):
        move = random.choice([[0,1], [1,0], [-1,0], [0,-1]])
        try_x, try_y = self.robotloc[0] + move[0], self.robotloc[1] + move[1]
        if self.test_valid(try_x, try_y):
            self.robotloc = [try_x, try_y]
